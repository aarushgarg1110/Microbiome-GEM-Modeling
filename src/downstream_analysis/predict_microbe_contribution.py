...
def _get_exchange_reactions(model: object, mets_list: Optional[List[str]] = None) -> List[str]:
    ...
    if not mets_list:
        ...
        return [rxn.id for rxn in model.reactions if 'IEX_' in rxn.id]
    else:
        # Only reactions matching provided metabolites
        return [rxn.id for rxn in model.reactions if any(m in rxn.id for m in mets_list)]
...
def _process_batch_parallel(current_batch: List[Path], diet_mod_dir: str, mets_list: Optional[List[str]], 
                           solver: str, workers: int) -> Dict:
    """Process batch of models in parallel"""
    batch_results = {}
    with ProcessPoolExecutor(max_workers=workers) as executor:
        futures = [
            executor.submit(_process_single_model, model_file, diet_mod_dir, mets_list, solver)
...
def predict_microbe_contributions(diet_mod_dir: str, res_path: Optional[str] = None, 
                                  mets_list: Optional[List[str]] = None, solver: str = 'cplex',
    ...
    for batch_start in range(0, len(remaining_models), batch_size):
        ...
        batch_results = _process_batch_parallel(current_batch, diet_mod_dir, mets_list, solver, workers)
